...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch service schedule' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Service schedule deleted successfully' }, { status: 200 });
  } catch (error) {
    console.error('Error deleting service schedule:', error);
    return NextResponse.json({ error: 'Failed to delete service schedule' }, { status: 500 });
  }
}
