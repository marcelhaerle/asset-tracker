...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (
      recordExists.serviceSchedule.lastServiceDate &&
      recordExists.serviceDate.getTime() ===
        recordExists.serviceSchedule.lastServiceDate.getTime() &&
      body.serviceDate
    ) {
      ...
      nextServiceDate.setMonth(
        nextServiceDate.getMonth() + recordExists.serviceSchedule.intervalMonths
      );
...
export async function DELETE(
  ...
  try {
    ...
    if (
      recordExists.serviceSchedule.serviceRecords.length > 1 &&
    ...
    return NextResponse.json({ message: 'Service record deleted successfully' }, { status: 200 });
  } catch (error) {
